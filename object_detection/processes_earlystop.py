...
def infer(args, unlabeled, ckpt_file):
    ...
    database2 = os.path.join(args['EXPT_DIR'], 'infer_preds_{}.db'.format(args['cur_loop']))
    conn = create_database(database)
    conn2 = create_database(database2)
    
    predix = 0

    unlabeledList = [i for i in unlabeled]
    batch_size = args["batch_size"]
    datamap = getdatasetstate(
        args, split="train"
    )  ##### Since our dataset object accepts list of imagenames we are using the state function again
